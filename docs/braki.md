...
| 8 | LTC audio reader (prawdziwy) | ✅ Hardware AJA, odczyt z karty dźwiękowej | ⚠️ MTC działa, LTC audio planowany (Faza 41) | MTC (MIDI Timecode) jest w pełni zaimplementowany — otwiera port MIDI Input, parsuje Quarter Frame messages, dekoduje TC (24/25/29.97/30fps). LTC audio (odczyt analogowego sygnału timecode z wejścia karty dźwiękowej) zaplanowany w **Fazie 41** — Web Audio API + AudioWorklet z dekoderem biphasowym SMPTE 12M. Nie wymaga hardware AJA — dowolna karta dźwiękowa USB. Tryb internal i manual działają poprawnie. |
